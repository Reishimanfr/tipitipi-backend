...
function RenderMobileMenu(render: boolean, shouldRender: boolean, setShouldRender: React.Dispatch<React.SetStateAction<boolean>>) {
  useEffect(() => {
    if (!render) {
  ...
  if (shouldRender) {
    return (
      <div className={`float-right fixed right-0 top-20 ${render ? 'animate-slide-in fill-forwards' : 'animate-slide-out fill-forwards'}`}>
  ...
  return <div className="hidden lg:flex"><RenderNavbar mobile={false}/></div>;
}
...
function Navbar() {
  ...
  return (
    <div className="w-full h-20 bg-black sticky top-0">
      <div>
        <Link to='/'>
          <img src={logoSmall} alt="logo" className="w-auto h-16 float-left ml-[5%] mt-[1vh]">
          </img>
        </Link>
        <button className="float-right h-20 lg:hidden" onClick={_ => setMenuVisible(!menuVisible)}>
